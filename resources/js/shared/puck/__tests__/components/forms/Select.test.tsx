...
defineBlockTestSuite({
  ...
  load: async () => {
    ...
    return { componentConfig: module.Select, componentRender: module.Select.render };
  },
  defaultProps,
  className: 'select-select-1',
  rootTag: 'div',
  allowedHardcodedValues: ['width: 100%', '0.2s', '999', '#3b82f6', '#000000', '#e5e7eb', '8px 12px', '14px', '10px 14px', '16px', '14px 18px', '18px'],
  extraTests: ({ renderWithDefaults }) => {
    describe('Select-specific tests', () => {
      it('should render label', () => {
        const { container } = renderWithDefaults();
        expect(container.textContent).toContain('Country');
      });

      it('should render select button with placeholder', () => {
        const { container } = renderWithDefaults();
        expect(container.textContent).toContain('Select a country');
      });

      it('should handle empty options', () => {
        const { container } = renderWithDefaults({ options: [] });
        expect(container.textContent).toContain('Select a country');
      });
