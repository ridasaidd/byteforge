...
export interface NavigationProps {
  ...
  // Placeholder for theme templates
...
export const Navigation: ComponentConfig<NavigationProps> = {
  ...
  fields: {
    ...
    placeholderItems: {
      type: 'array',
      label: 'Placeholder Menu Items (Theme Templates)',
      arrayFields: {
...
function NavigationRenderer({
  ...
  if (!navigationId || !navigation) {
    // If no navigation is selected, check if we have placeholder items
    if (placeholderItems && placeholderItems.length > 0) {
      // We have placeholder items, so we proceed to render them instead of the empty state
    } else {
      return (
        <div
  ...
  // 1. If we have a loaded navigation, use its structure (built into hierarchy)
  // 2. If not, fallback to placeholder items implies we are in placeholder mode
  const rawItems = navigation?.structure
    ? buildHierarchy(navigation.structure)
    : (placeholderItems || []);

  // Ensure placeholder items have IDs if they are missing
  const ensureIds = (items: MenuItem[]): MenuItem[] => {
    return items.map((item, i) => ({
      ...item,
      id: item.id || `ph-${i}-${Math.random().toString(36).substr(2, 5)}`,
      children: item.children ? ensureIds(item.children) : undefined
    }));
  };

  const menuItems = navigation?.structure ? rawItems : ensureIds(rawItems);
