...
export interface MinHeightValue {
  value: string;
...
interface MinHeightControlProps {
  field: { label?: string };
  value: MinHeightValue | string | undefined;
  onChange: (value: MinHeightValue) => void;
}
...
export function MinHeightControl({
  ...
  // Normalize value to MinHeightValue format
  const normalizedValue: MinHeightValue = typeof value === 'string'
    ? parseMinHeightString(value)
  ...
  const handleUnitChange = (newUnit: MinHeightValue['unit']) => {
    if (newUnit === 'auto') {
      ...
      <div style={{ display: 'flex', gap: '3px', flexWrap: 'wrap' }}>
        {unitOptions.map(({ value: unitValue, label: unitLabel }) => (
          <button
            key={unitValue}
            type="button"
            onClick={() => handleUnitChange(unitValue as MinHeightValue['unit'])}
            style={{
...
// Helper function to parse minHeight string to MinHeightValue
function parseMinHeightString(str: string): MinHeightValue {
  if (str === 'auto' || !str) {
  ...
  if (match) {
    return {
      value: match[1],
      unit: match[2] as MinHeightValue['unit'],
    };
  }
