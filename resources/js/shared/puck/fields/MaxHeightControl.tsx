...
export interface MaxHeightValue {
  value: string;
  unit: 'px' | '%' | 'rem' | 'em' | 'vh' | 'vw' | 'none';
}

interface MaxHeightControlProps {
  field: { label?: string };
  value: MaxHeightValue | string | undefined;
  onChange: (value: MaxHeightValue) => void;
}

export function MaxHeightControl({
  field,
  value,
  onChange,
}: MaxHeightControlProps) {
  // Normalize value to MaxHeightValue format
  const normalizedValue: MaxHeightValue = typeof value === 'string'
    ? parseMaxHeightString(value)
  ...
  const handleUnitChange = (newUnit: MaxHeightValue['unit']) => {
    if (newUnit === 'none') {
      ...
      <div style={{ display: 'flex', gap: '3px', flexWrap: 'wrap' }}>
        {unitOptions.map(({ value: unitValue, label: unitLabel }) => (
          <button
            key={unitValue}
            type="button"
            onClick={() => handleUnitChange(unitValue as MaxHeightValue['unit'])}
            style={{
...
// Helper function to parse maxHeight string to MaxHeightValue
function parseMaxHeightString(str: string): MaxHeightValue {
  if (str === 'none' || !str) {
  ...
  if (match) {
    return {
      value: match[1],
      unit: match[2] as MaxHeightValue['unit'],
    };
  }
